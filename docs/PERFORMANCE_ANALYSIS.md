...
---
# Матриця кешування CRM

### Загальні правила

| Правило | Пояснення |
|---------|-----------|
| **Не кешувати real-time дані** | Баланси, статуси, нові транзакції вимагають актуальності |
| **Кешувати довідники** | Список сервісів, ролей, валют змінюються рідко |
| **Кешувати важкі агрегати** | Статистика, звіти, аналітика |
| **TTL залежить від частоти змін** | Чим частіше зміни - тим коротший TTL |
| **Інвалідація при зміні** | Кеш очищається при створенні/оновленні/видаленні |

### Матриця: Кеш / Не кешувати

| Дані | Кешувати? | TTL / умова інвалідації | Причина |
|------|-----------|-------------------------|---------|
| **Список валют (currencies)** | ✅ Так | TTL: 24 години | Довідник, майже не змінюється |
| **Список послуг (services)** | ✅ Так | TTL: 30 хвилин | Довідник, рідко змінюється |
| **Ролі та права (roles/permissions)** | ✅ Так | TTL: 1 година + інвалідація при зміні | Довідник, важлива консистентність |
| **Налаштування комісій** | ✅ Так | TTL: 1 година | Рідко змінюється, допустима затримка |
| **Статистика дашборду** | ✅ Так | TTL: 5-15 хвилин + інвалідація при подіях | Важкі обчислення, допустима затримка |
| **Топ клієнтів за балансом** | ✅ Так | TTL: 30 хвилин | Частина дашборду |
| **Статуси інвойсів (enum)** | ✅ Так | TTL: 24 години | Довідник |
| **Типи транзакцій (enum)** | ✅ Так | TTL: 24 години | Довідник |
| **Баланс рахунку (account.balance)** | ❌ Ні | - | Real-time, критична консистентність |
| **Список переказів для рахунку** | ❌ Ні (або 1 хв) | - | Часто змінюється, персоналізовані |
| **Список інвойсів для клієнта** | ❌ Ні | - | Часто оновлюється |
| **Один переказ за ID** | ❌ Ні | - | Може щойно створений |
| **Один інвойс за ID** | ❌ Ні | - | Може бути змінений |
| **Транзакція за ID** | ❌ Ні | - | Фінансові дані, консистентність критична |
| **Поточний користувач (auth->me)** | ❌ Ні | - | Ролі та дозволи змінюються |
| **Підтвердження переказу** | ❌ Ні | - | Повинне бути актуальним |

### Кандидати на кешування (CRM специфічні)

#### 1. Агрегована статистика за місяць
```php
Cache::remember('transactions:summary:' . $month, 900, function() {
...
#### 2. Клієнтський портфель (сума балансів всіх рахунків)
```php
Cache::remember('client:portfolio:' . $clientId, 600, function() use ($clientId) {
    return Account::where('client_id', $clientId)->sum('balance');
});
```

#### 3. Список активних клієнтів (з пагінацією)
```php
Cache::remember('clients:active:page:' . $page, 300, function() {
    return Client::where('is_active', true)->paginate(15);
});
```

### Дані, які НЕ можна кешувати

#### 1. Баланс рахунку після переказу
```php
// ❌ НЕ КЕШУВАТИ
$balance = Cache::remember('account:balance:' . $id, 3600, function() use ($id) {
    return Account::find($id)->balance;
});

// ✅ КОРИСТУВАТИ ПРЯМИЙ ЗАПИТ
$balance = Account::find($id)->balance;
```

#### 2. Статус інвойсу після оплати
```php
// ❌ НЕ КЕШУВАТИ - статус може змінитися в будь-який момент
// ✅ Завжди брати з БД
$invoice = Invoice::find($id);
$status = $invoice->status;
```

#### 3. Посилання на щойно створений переказ
```php
// ❌ НЕ КЕШУВАТИ - клієнт очікує негайне підтвердження
// ✅ Повертати з БД
```

### Інвалідація кешу

| Подія | Який кеш інвалідувати |
|-------|----------------------|
| Створення клієнта | `clients:list`, `dashboard:stats` |
| Створення переказу | `dashboard:stats`, `account:balance:*` |
| Оновлення статусу інвойсу | `invoices:*`, `dashboard:stats` |
| Створення послуги | `services:list` |
...
class TransferService
{
    public function executeTransfer(TransferDTO $dto): array
    {
        ...
        // Інвалідація кешу після операції
        Cache::forget('dashboard:stats');
        Cache::forget('account:transactions:' . $dto->accountFromId);
        Cache::forget('account:transactions:' . $dto->accountToId);
...
| Тип даних | Підхід |
|-----------|--------|
| **Довідники** | Кешувати на довго (30 хв - 24 год) |
